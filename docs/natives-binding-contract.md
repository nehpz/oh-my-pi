...
There is no `NativeBindings` declaration-merging lifecycle or `packages/natives/src/<module>` wrapper convention. The loader validates only a release-version sentinel for every load, not every public symbol; a function export the loaded addon omits is `missingNativeExport(name)` — `undefined` on a current addon, and on a stale addon a throwing stub that names the addon and the rebuild command (`bun run build:native`).
...
- Candidates missing the expected version sentinel are rejected during loading, including workspace-development candidates.
