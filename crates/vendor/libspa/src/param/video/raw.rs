...
impl VideoInfoRaw {
    pub fn new() -> Self {
        // SAFETY: `spa_video_info_raw` is a C data struct containing only integer-backed
        // enums, integers, and nested integer structs. Zero initializes every field to
        // the documented unknown/default state on both sides of the 0.3.65 layout change.
    ...
    pub fn set_flags(&mut self, flags: VideoFlags) {
        ...
        #[cfg(not(libspa_video_info_has_flags))]
        let _ = flags;
    }
    ...
    pub fn flags(self) -> VideoFlags {
        #[cfg(libspa_video_info_has_flags)]
        {
            return VideoFlags::from_bits_retain(self.0.flags);
        }
        #[cfg(not(libspa_video_info_has_flags))]
        VideoFlags::empty()
    }
